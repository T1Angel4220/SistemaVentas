...
class SavedProductsController {
  ...
  static async saveProduct(req, res) {
    try {
      ...
      const productoExistente = await query(
        'SELECT * FROM items WHERE id = $1 AND estado = $2',
        [id, 'activo']
      );
      ...
      // Guardar producto
      const productoGuardado = await query(
        `INSERT INTO productos_guardados (usuario_id, item_id)
         VALUES ($1, $2)
         RETURNING *`,
        [usuario_id, id]
      );
  ...
  static async unsaveProduct(req, res) {
    try {
      ...
      const productoGuardado = await query(
        'SELECT * FROM productos_guardados WHERE usuario_id = $1 AND item_id = $2',
        [usuario_id, id]
      );
  ...
  static async getSavedProducts(req, res) {
    try {
      ...
      const productosGuardados = await query(
        `SELECT 
  ...
  static async checkIfSaved(req, res) {
    try {
      ...
      const productoGuardado = await query(
        'SELECT id, fecha_guardado FROM productos_guardados WHERE usuario_id = $1 AND item_id = $2',
        [usuario_id, id]
      );
