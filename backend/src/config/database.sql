...
-- Tipos de usuario
CREATE TYPE tipo_usuario AS ENUM ('comprador', 'vendedor', 'moderador', 'administrador');

-- Estados de usuario
CREATE TYPE estado_usuario AS ENUM ('activo', 'inactivo', 'suspendido', 'pendiente_verificacion');

-- Tipos de producto/servicio
CREATE TYPE tipo_item AS ENUM ('producto', 'servicio');

-- Estados de producto/servicio
CREATE TYPE estado_item AS ENUM ('activo', 'inactivo', 'pendiente_revision', 'rechazado', 'peligroso', 'suspendido');

-- Estados de reporte/incidencia
CREATE TYPE estado_reporte AS ENUM ('pendiente', 'en_revision', 'resuelto', 'rechazado', 'en_apelacion');

-- Tipos de reporte
CREATE TYPE tipo_reporte AS ENUM ('contenido_inapropiado', 'producto_prohibido', 'informacion_falsa', 'spam', 'otro');
...
CREATE TABLE servicios (
    ...
    dias_disponibles VARCHAR(100), -- ej: "Lunes-Viernes"
    duracion_estimada VARCHAR(50), -- ej: "2 horas"
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE acciones_moderacion (
    ...
    accion VARCHAR(100) NOT NULL, -- ej: "suspender_usuario", "aprobar_producto", "rechazar_producto"
    tabla_afectada VARCHAR(50), -- ej: "usuarios", "items", "reportes"
...
CREATE TRIGGER trigger_items_actualizacion
    BEFORE UPDATE ON items
    FOR EACH ROW
    EXECUTE FUNCTION actualizar_fecha_modificacion();

-- Función para registrar acciones de moderación
CREATE OR REPLACE FUNCTION registrar_accion_moderacion()
RETURNS TRIGGER AS $$
BEGIN
    -- Solo registrar si el usuario es moderador o administrador
    IF (SELECT tipo_usuario FROM usuarios WHERE id = NEW.moderador_asignado_id) IN ('moderador', 'administrador') THEN
...
CREATE TRIGGER trigger_registrar_asignacion_reporte
    AFTER UPDATE OF moderador_asignado_id ON reportes
    FOR EACH ROW
    WHEN (NEW.moderador_asignado_id IS NOT NULL AND OLD.moderador_asignado_id IS NULL)
...
WHERE i.estado = 'activo' AND i.disponibilidad = TRUE
GROUP BY i.id, i.codigo, i.nombre, i.descripcion, i.precio, i.tipo, 
         i.fecha_publicacion, c.nombre, u.nombre, u.apellido, u.correo, ub.nombre;
...
SELECT 
    ...
    COUNT(CASE WHEN email_verificado = TRUE THEN 1 END) as usuarios_verificados
FROM usuarios
