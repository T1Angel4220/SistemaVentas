...
async function createHierarchicalCategories() {
  ...
  try {
    ...
    // Primero, actualizar los items que referencian categorías para evitar violaciones de llave foránea
    console.log('  ⚠️ Actualizando items que referencian categorías...');
    const updateResult = await client.query('UPDATE items SET categoria_id = NULL WHERE categoria_id IS NOT NULL');
    console.log(`  ✅ ${updateResult.rowCount} items actualizados (categoria_id = NULL)`);
    
    // Ahora sí podemos eliminar las categorías
