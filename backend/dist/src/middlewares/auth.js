...
const authenticate = async (req, res, next) => {
    try {
        ...
        const userResult = await query('SELECT id, correo, tipo_usuario, estado, email_verificado FROM usuarios WHERE id = $1', [decoded.id]);
        ...
        const sessionResult = await query('SELECT id FROM sesiones_usuario WHERE usuario_id = $1 AND activa = true AND fecha_expiracion > NOW() LIMIT 1', [decoded.id]);
...
const requireProductPermission = (action) => {
    return (req, res, next) => {
        ...
        if (userPermissions[action] === 'own') {
            ...
            return query('SELECT vendedor_id FROM items WHERE id = $1', [productId]).then(result => {
    ...
    return (req, res, next) => {
        ...
        if (req.user.tipo_usuario === 'moderador') {
            ...
            if (targetUserId && targetUserId !== req.user.id.toString()) {
                ...
                return query('SELECT tipo_usuario FROM usuarios WHERE id = $1', [targetUserId]).then(result => {
...
const requireActiveSession = async (req, res, next) => {
    try {
        ...
        const sessionResult = await query('SELECT id FROM sesiones_usuario WHERE usuario_id = $1 AND activa = true AND fecha_expiracion > NOW()', [req.user.id]);
        if (sessionResult.rows.length === 0) {
            return res.status(401).json({
                success: false,
