...
router.put('/activate-user/:userId', authenticate, requireModerator, async (req, res) => {
    try {
        ...
        const userResult = await require('../config/database').query('SELECT * FROM usuarios WHERE id = $1', [userId]);
        ...
        await require('../config/database').query('UPDATE usuarios SET estado = $1 WHERE id = $2', ['activo', userId]);
    ...
    `, [
        ...
        try {
            ...
            await require('../services/email').sendAccountStatusEmail(user.correo, nombreCompleto, 'activo', motivo || 'Tu cuenta ha sido reactivada exitosamente');
...
router.put('/deactivate-user/:userId', authenticate, requireModerator, async (req, res) => {
    try {
        ...
        // Actualizar estado del usuario
        await require('../config/database').query('UPDATE usuarios SET estado = $1 WHERE id = $2', ['inactivo', userId]);
        // Invalidar todas las sesiones del usuario
        await require('../config/database').query('UPDATE sesiones_usuario SET activa = false WHERE usuario_id = $1', [userId]);
        // Registrar acción de moderación
        await require('../config/database').query(`
      INSERT INTO acciones_moderacion (moderador_id, accion, tabla_afectada, registro_id, detalles)
      VALUES ($1, $2, $3, $4, $5)
    `, [
        ...
        try {
            await require('../services/email').sendAccountStatusEmail(user.correo, user.nombre, 'inactivo', motivo || 'Tu cuenta ha sido desactivada');
        }
        catch (emailError) {
...
router.put('/suspend-user/:userId', authenticate, requireModerator, async (req, res) => {
    try {
        ...
        // Actualizar estado del usuario
        await require('../config/database').query('UPDATE usuarios SET estado = $1 WHERE id = $2', ['suspendido', userId]);
        // Invalidar todas las sesiones del usuario
        await require('../config/database').query('UPDATE sesiones_usuario SET activa = false WHERE usuario_id = $1', [userId]);
        // Registrar acción de moderación
        await require('../config/database').query(`
    ...
    `, [
        ...
        try {
            const nombreCompleto = `${user.nombre || ''} ${user.apellido || ''}`.trim() || user.correo;
            await require('../services/email').sendAccountStatusEmail(user.correo, nombreCompleto, 'suspendido', motivo || 'Tu cuenta ha sido suspendida por incumplimiento de las políticas de uso');
...
router.delete('/sessions/:sessionId', authenticate, async (req, res) => {
    try {
        ...
        const sessionResult = await require('../config/database').query('SELECT id FROM sesiones_usuario WHERE id = $1 AND usuario_id = $2', [sessionId, userId]);
        ...
        // Cerrar sesión
        await require('../config/database').query('UPDATE sesiones_usuario SET activa = false WHERE id = $1', [sessionId]);
        res.json({
            success: true,
...
router.get('/admin/sessions/:userId', authenticate, requireModerator, async (req, res) => {
    try {
        const { userId } = req.params;
        // Verificar que el usuario existe
        const userResult = await require('../config/database').query('SELECT id, nombre, apellido, correo, tipo_usuario FROM usuarios WHERE id = $1', [userId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Usuario no encontrado'
...
router.delete('/admin/sessions/user/:userId', authenticate, requireModerator, async (req, res) => {
    try {
        ...
        const activeSessionsResult = await require('../config/database').query('SELECT COUNT(*) as count FROM sesiones_usuario WHERE usuario_id = $1 AND activa = true', [userId]);
        ...
        await require('../config/database').query('UPDATE sesiones_usuario SET activa = false WHERE usuario_id = $1 AND activa = true', [userId]);
        // Registrar acción de moderación
        await require('../config/database').query(`
      INSERT INTO acciones_moderacion (moderador_id, accion, tabla_afectada, registro_id, detalles)
      VALUES ($1, $2, $3, $4, $5)
