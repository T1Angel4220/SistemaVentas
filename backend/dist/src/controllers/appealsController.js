...
class AppealsController {
    ...
    static async createAppeal(req, res) {
        try {
            ...
            const apelacionExistente = await query('SELECT * FROM apelaciones WHERE item_id = $1 AND estado IN ($2, $3)', [item_id, 'en_apelacion', 'pendiente']);
            ...
            // Crear la apelación
            const result = await query(`INSERT INTO apelaciones 
        (item_id, usuario_apelante_id, motivo_apelacion, informacion_adicional, estado, fecha_apelacion)
        VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)
        RETURNING *`, [item_id, usuario_apelante_id, motivo_apelacion, informacion_adicional || null, 'en_apelacion']);
            // Actualizar el estado del producto a "en_apelacion" si estaba rechazado o suspendido
            if (producto.estado === 'rechazado' || producto.estado === 'suspendido') {
                await query('UPDATE items SET estado = $1 WHERE id = $2', ['en_apelacion', item_id]);
            }
            res.status(201).json({
                success: true,
    ...
    static async getAppealsByProduct(req, res) {
        ...
        WHERE a.item_id = $1
        ORDER BY a.fecha_apelacion DESC`, [item_id]);
            res.json({
                success: true,
                data: result.rows
            });
    ...
    static async resolveAppeal(req, res) {
        try {
            ...
            const apelacionResult = await query('SELECT * FROM apelaciones WHERE id = $1', [appeal_id]);
            ...
            await query(`UPDATE apelaciones 
        SET estado = $1,
        ...
        WHERE id = $4`, [nuevoEstadoApelacion, decision_apelacion, moderador_revisor_id, appeal_id]);
          ...
          SET estado = $1, 
              ...
              es_peligroso = FALSE,
              disponibilidad = TRUE
          WHERE id = $3`, [nuevoEstadoProducto, moderador_revisor_id, apelacion.item_id]);
            }
            else {
          ...
          WHERE id = $3`, [nuevoEstadoProducto, moderador_revisor_id, apelacion.item_id]);
            ...
            const apelacionActualizada = await query(`SELECT 
          a.*,
          i.nombre as producto_nombre,
          i.estado as producto_estado,
          u_apelante.nombre as apelante_nombre,
    ...
    static async getMyAppeals(req, res) {
        ...
        ORDER BY a.fecha_apelacion DESC`, [usuario_id]);
            res.json({
                success: true,
                data: result.rows,
                count: result.rows.length
