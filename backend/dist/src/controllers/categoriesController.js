...
class CategoriesController {
    ...
    static async getCategories(req, res) {
        try {
            ...
            const categorias = await query(`SELECT id, nombre, descripcion, activa, fecha_creacion, 
                categoria_padre_id, nivel, orden
         FROM categorias 
         WHERE activa = $1
         ORDER BY nivel ASC, orden ASC, nombre ASC`, [activa]);
            res.json({
                success: true,
    ...
    static async getCategoryById(req, res) {
        try {
            const { id } = req.params;
            const categoria = await query('SELECT * FROM categorias WHERE id = $1', [id]);
    ...
    static async createCategory(req, res) {
        try {
            ...
            const categoriaExistente = await query('SELECT id FROM categorias WHERE nombre = $1', [nombre]);
            ...
            // Crear categoría
            const nuevaCategoria = await query(`INSERT INTO categorias (nombre, descripcion, activa)
         VALUES ($1, $2, true)
    ...
    static async updateCategory(req, res) {
        try {
            ...
            const categoriaExistente = await query('SELECT * FROM categorias WHERE id = $1', [id]);
            if (categoriaExistente.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'Categoría no encontrada'
            ...
            if (nombre && nombre !== categoriaExistente.rows[0].nombre) {
                const nombreExistente = await query('SELECT id FROM categorias WHERE nombre = $1 AND id != $2', [nombre, id]);
            ...
            const categoriaActualizada = await query(`UPDATE categorias SET 
          nombre = COALESCE($1, nombre),
          descripcion = COALESCE($2, descripcion),
          activa = COALESCE($3, activa)
        WHERE id = $4
        RETURNING *`, [nombre, descripcion, activa, id]);
            res.json({
                success: true,
    ...
    static async deleteCategory(req, res) {
        try {
            ...
            const productosConCategoria = await query('SELECT COUNT(*) as total FROM items WHERE categoria_id = $1', [id]);
