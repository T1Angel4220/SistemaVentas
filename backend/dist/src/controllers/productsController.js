...
class ProductsController {
    ...
    static async createProduct(req, res) {
        try {
            ...
            const { codigo, nombre, descripcion, precio, ubicacion_id, ubicacion_provincia, ubicacion_canton, ubicacion_distrito, ubicacion_direccion, coordenadas, tipo, categoria_id, horario_atencion, dias_disponibles, duracion_estimada } = req.body;
            ...
            const codigoExistente = await query('SELECT id FROM items WHERE codigo = $1', [codigo]);
            ...
            if (ubicacion_provincia && ubicacion_canton) {
                try {
                    ...
                    const ubicacionExistente = await query('SELECT id FROM ubicaciones WHERE provincia = $1 AND canton = $2 LIMIT 1', [ubicacion_provincia, ubicacion_canton]);
            ...
            const nuevoProducto = await query(`INSERT INTO items (
        ...
        RETURNING *`, [codigo, nombre, descripcion, precio, ubicacionIdFinal,
                ubicacion_provincia, ubicacion_canton, ubicacion_distrito, ubicacion_direccion, coordenadas,
            ...
            if (tipo === 'servicio' && horario_atencion) {
                await query(`INSERT INTO servicios (item_id, horario_atencion, dias_disponibles, duracion_estimada)
           VALUES ($1, $2, $3, $4)`, [producto.id, horario_atencion, dias_disponibles, duracion_estimada]);
            }
            ...
            if (req.files && req.files.length > 0) {
                ...
                for (let i = 0; i < req.files.length; i++) {
                    ...
                    await query(`INSERT INTO item_imagenes (item_id, url_imagen, orden, es_principal)
             VALUES ($1, $2, $3, $4)`, [producto.id, buildImageUrl(file.filename), i + 1, esPrincipal]);
                }
    ...
    static async getProducts(req, res) {
        try {
            const { categoria_id, tipo, precio_min, precio_max, ubicacion_id, provincia, canton, distrito, direccion, estado = 'activo', disponibilidad = true, page = 1, limit = 10, search, 
            ...
            const productos = await query(`SELECT 
        ...
        LIMIT $${paramCount - 1} OFFSET $${paramCount}`, queryParams);
         ...
         FROM items i
         WHERE ${whereClause}`, queryParams.slice(0, -2) // Remover limit y offset
            );
    ...
    static async getProductById(req, res) {
        try {
            const { id } = req.params;
            const producto = await query(`SELECT 
          i.*,
          c.nombre as categoria_nombre,
        ...
        WHERE i.id = $1`, [id]);
            ...
            const imagenes = await query('SELECT * FROM item_imagenes WHERE item_id = $1 ORDER BY orden', [id]);
            ...
            if (producto.rows[0].tipo === 'servicio') {
                const servicio = await query('SELECT * FROM servicios WHERE item_id = $1', [id]);
    ...
    static async updateProduct(req, res) {
        try {
            ...
            const { nombre, descripcion, precio, ubicacion_id, ubicacion_provincia, ubicacion_canton, ubicacion_distrito, ubicacion_direccion, coordenadas, categoria_id, horario_atencion, dias_disponibles, duracion_estimada } = req.body;
            // Verificar que el producto existe
            const productoExistente = await query('SELECT * FROM items WHERE id = $1', [id]);
            ...
            const productoActualizado = await query(`UPDATE items SET 
        ...
        WHERE id = $15
        RETURNING *`, [nombre, descripcion, precio, ubicacionIdFinal,
          ...
          WHERE item_id = $4`, [horario_atencion, dias_disponibles, duracion_estimada, id]);
            }
            ...
            if (req.body.deleted_images) {
                try {
                    ...
                    if (Array.isArray(deletedImageIndices) && deletedImageIndices.length > 0) {
                        ...
                        const allImages = await query('SELECT id, url_imagen FROM item_imagenes WHERE item_id = $1 ORDER BY orden ASC', [id]);
                        ...
                        if (idsToDelete.length > 0) {
                            ...
                            const imagesToDelete = await query('SELECT url_imagen FROM item_imagenes WHERE id = ANY($1)', [idsToDelete]);
                            console.log('📁 URLs de imágenes a eliminar:', imagesToDelete.rows);
                            // Eliminar de la base de datos
                            await query('DELETE FROM item_imagenes WHERE id = ANY($1)', [idsToDelete]);
            ...
            if (req.files && req.files.length > 0) {
                ...
                for (let i = 0; i < req.files.length; i++) {
                    ...
                    const nextOrder = await query('SELECT COALESCE(MAX(orden), 0) + 1 as next_order FROM item_imagenes WHERE item_id = $1', [id]);
                    await query(`INSERT INTO item_imagenes (item_id, url_imagen, orden, es_principal)
             VALUES ($1, $2, $3, $4)`, [id, buildImageUrl(file.filename), nextOrder.rows[0].next_order, false]);
                }
            }
    ...
    static async getMyProducts(req, res) {
        ...
        GROUP BY i.id, i.codigo, i.nombre, i.descripcion, i.precio, 
                 ...
                 c.nombre
        ORDER BY i.fecha_publicacion DESC
        LIMIT ${limitPlaceholder} OFFSET ${offsetPlaceholder}`, queryParams);
            // Contar total (sin limit y offset)
            const countParams = queryParams.slice(0, -2);
            const totalCount = await query(`SELECT COUNT(*) as total
         FROM items i
         WHERE ${whereClause}`, countParams);
            const total = parseInt(totalCount.rows[0].total);
            const totalPages = Math.ceil(total / limit);
            res.json({
                success: true,
        ...
        try {
            ...
            switch (accion) {
                ...
                case 'marcar_revisado_detalle':
                    ...
                    await query(`UPDATE items SET fecha_revision = NOW() WHERE id = $1`, [id]);
          ...
          disponibilidad = $7
        WHERE id = $6
        RETURNING *`, [nuevoEstado, esPeligroso, fechaDeteccionPeligroso, moderador_id, motivo, id, disponibilidad]);
            // Registrar acción de moderación
            await query(`INSERT INTO acciones_moderacion (moderador_id, accion, tabla_afectada, registro_id, detalles)
         VALUES ($1, $2, 'items', $3, $4)`, [moderador_id, `moderar_producto_${accion}`, id, decision_final || motivo]);
            // Si se marcó como peligroso, verificar si se debe bloquear la cuenta del vendedor
            if (esPeligroso && producto.vendedor_id) {
                const bloqueoResult = await ProductsController.verificarYBloquearCuentaPorProductosPeligrosos(producto.vendedor_id);
                if (bloqueoResult.bloqueado) {
    ...
    static async saveProduct(req, res) {
        try {
            ...
            const productCheck = await query('SELECT id, estado FROM items WHERE id = $1', [productoId]);
            ...
            const alreadySaved = await query('SELECT id FROM productos_guardados WHERE usuario_id = $1 AND item_id = $2', [userId, productoId]);
            ...
            // Guardar el producto
            await query('INSERT INTO productos_guardados (usuario_id, item_id, fecha_guardado) VALUES ($1, $2, NOW())', [userId, productoId]);
    ...
    static async unsaveProduct(req, res) {
        try {
            ...
            const savedProduct = await query('SELECT id FROM productos_guardados WHERE usuario_id = $1 AND item_id = $2', [userId, productoId]);
            ...
            await query('DELETE FROM productos_guardados WHERE usuario_id = $1 AND item_id = $2', [userId, productoId]);
            return res.status(200).json({
                success: true,
    ...
    static async getSavedStatus(req, res) {
        try {
            ...
            const result = await query('SELECT id FROM productos_guardados WHERE usuario_id = $1 AND item_id = $2', [userId, productoId]);
            return res.status(200).json({
                success: true,
    ...
    static async getMyDangerousProducts(req, res) {
        ...
        ORDER BY i.fecha_deteccion_peligroso DESC`, [vendedor_id]);
            res.json({
                success: true,
                data: result.rows
            });
    ...
    static async verificarYBloquearCuentaPorProductosPeligrosos(vendedor_id) {
        try {
            ...
            const countResult = await query(`SELECT COUNT(*) as cantidad 
         FROM items 
         WHERE vendedor_id = $1 
         AND es_peligroso = true`, [vendedor_id]);
            ...
            if (cantidadPeligrosos >= 3) {
                ...
                const userResult = await query(`SELECT id, correo, nombre, apellido, tipo_usuario, estado 
           FROM usuarios 
           WHERE id = $1`, [vendedor_id]);
             ...
             WHERE id = $1`, [vendedor_id]);
                    ...
                    await query(`UPDATE sesiones_usuario 
             SET activa = false 
             WHERE usuario_id = $1`, [vendedor_id]);
                    // Enviar email de notificación
                    try {
                        const nombreCompleto = `${user.nombre || ''} ${user.apellido || ''}`.trim() || user.correo;
                        await sendAccountBlockedByDangerousProductsEmail(user.correo, nombreCompleto, cantidadPeligrosos);
    ...
    static async suspenderProductosExpirados() {
        try {
            ...
            const productosExpirados = await query(`SELECT 
          id, 
          codigo, 
          nombre, 
          vendedor_id,
        ...
        ORDER BY fecha_publicacion ASC`);
            ...
            for (const producto of productosExpirados.rows) {
             ...
             SET estado = 'suspendido',
                 ...
                 fecha_revision = CURRENT_TIMESTAMP,
                 fecha_actualizacion = CURRENT_TIMESTAMP
             WHERE id = $2`, [motivoSuspension, producto.id]);
                    // Registrar acción de moderación automática
                    // Usamos moderador_id = null para indicar que fue automático
                    await query(`INSERT INTO acciones_moderacion (moderador_id, accion, tabla_afectada, registro_id, detalles)
             VALUES (NULL, 'suspension_automatica_tiempo_expirado', 'items', $1, $2)`, [producto.id, `Producto suspendido automáticamente después de ${Math.round(producto.dias_pendiente)} días sin revisar`]);
