...
class ImageController {
    ...
    static async uploadImages(req, res) {
        try {
            ...
            const imagenesExistentes = await query('SELECT COUNT(*) as total FROM item_imagenes WHERE item_id = $1', [id]);
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                const imagenGuardada = await query(`INSERT INTO item_imagenes (item_id, url_imagen, orden, es_principal)
           VALUES ($1, $2, $3, $4)
           RETURNING *`, [id, urlImagen, orden, esPrincipal]);
    ...
    static async deleteImage(req, res) {
        try {
            ...
            const imagenExistente = await query('SELECT * FROM item_imagenes WHERE id = $1 AND item_id = $2', [imageId, id]);
            ...
            // Verificar permisos
            const producto = await query('SELECT vendedor_id FROM items WHERE id = $1', [id]);
            ...
            if (imagen.es_principal) {
                const siguienteImagen = await query('SELECT id FROM item_imagenes WHERE item_id = $1 ORDER BY orden LIMIT 1', [id]);
                if (siguienteImagen.rows.length > 0) {
                    await query('UPDATE item_imagenes SET es_principal = true WHERE id = $1', [siguienteImagen.rows[0].id]);
                }
            }
            // Reordenar las imágenes restantes
            await query(`UPDATE item_imagenes 
         SET orden = orden - 1 
         WHERE item_id = $1 AND orden > $2`, [id, imagen.orden]);
            res.json({
                success: true,
    ...
    static async setMainImage(req, res) {
        try {
            ...
            await query('UPDATE item_imagenes SET es_principal = false WHERE item_id = $1', [id]);
            // Marcar la imagen seleccionada como principal
            await query('UPDATE item_imagenes SET es_principal = true WHERE id = $1', [imageId]);
            res.json({
                success: true,
    ...
    static async reorderImages(req, res) {
        try {
            ...
            for (let i = 0; i < imageIds.length; i++) {
                await query('UPDATE item_imagenes SET orden = $1 WHERE id = $2 AND item_id = $3', [i + 1, imageIds[i], id]);
            }
            res.json({
                success: true,
