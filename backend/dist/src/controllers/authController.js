...
const { generateSessionTokens, generateEmailVerificationToken, generatePasswordResetToken, verifyEmailVerificationToken, verifyPasswordResetToken } = require('../services/jwt');
const { sendVerificationEmail, sendPasswordResetEmail, sendAccountStatusEmail, sendNewSessionEmail } = require('../services/email');
...
const register = async (req, res) => {
    try {
        ...
        const existingEmail = await query('SELECT id FROM usuarios WHERE correo = $1', [correo]);
        ...
        const existingCedula = await query('SELECT id FROM usuarios WHERE cedula = $1', [cedula]);
...
const login = async (req, res) => {
    try {
        ...
        const userResult = await query('SELECT * FROM usuarios WHERE correo = $1', [correo]);
    ...
    `, [
        ...
        await query('UPDATE usuarios SET fecha_ultimo_acceso = NOW() WHERE id = $1', [user.id]);
        // Enviar notificación de nueva sesión (opcional)
        try {
            await sendNewSessionEmail(user.correo, user.nombre, clientIp, req.get('User-Agent') || 'User-Agent desconocido');
        }
        catch (emailError) {
...
const resendVerificationCode = async (req, res) => {
    try {
        ...
        // Buscar usuario por email
        const userResult = await query('SELECT id, correo, nombre, estado, email_verificado FROM usuarios WHERE correo = $1', [correo]);
        if (userResult.rows.length === 0) {
        ...
        await query('UPDATE usuarios SET token_verificacion = $1, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = $2', [verificationToken, user.id]);
...
const logout = async (req, res) => {
    try {
        ...
        if (token) {
            await query('UPDATE sesiones_usuario SET activa = false WHERE usuario_id = $1 AND token_sesion = $2', [userId, token]);
        }
        res.json({
            success: true,
...
const requestPasswordReset = async (req, res) => {
    try {
        ...
        // Buscar usuario por email
        const userResult = await query('SELECT id, correo, nombre, apellido, estado FROM usuarios WHERE correo = $1', [correo]);
        if (userResult.rows.length === 0) {
            // Por seguridad, no revelamos si el email existe o no
            return res.json({
                success: true,
        ...
        await query('UPDATE usuarios SET token_recuperacion = $1, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = $2', [resetCode, user.id]);
...
const resetPassword = async (req, res) => {
    try {
        ...
        const userResult = await query('SELECT id, correo, nombre, apellido, password_hash, token_recuperacion, fecha_actualizacion FROM usuarios WHERE token_recuperacion = $1', [code]);
        if (userResult.rows.length === 0) {
            return res.status(400).json({
                success: false,
        ...
        if (timeDiff > 10) {
            ...
            await query('UPDATE usuarios SET token_recuperacion = NULL WHERE id = $1', [user.id]);
            return res.status(400).json({
                success: false,
        ...
        await query('UPDATE usuarios SET password_hash = $1, token_recuperacion = NULL, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = $2', [passwordHash, user.id]);
        // Invalidar todas las sesiones activas del usuario
        await query('UPDATE sesiones_usuario SET activa = false WHERE usuario_id = $1', [user.id]);
...
const changePassword = async (req, res) => {
    try {
        ...
        const userResult = await query('SELECT id, password_hash FROM usuarios WHERE id = $1', [userId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Usuario no encontrado'
        ...
        await query('UPDATE usuarios SET password_hash = $1, fecha_actualizacion = CURRENT_TIMESTAMP WHERE id = $2', [newPasswordHash, userId]);
