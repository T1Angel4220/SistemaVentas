...
class ReportsController {
    ...
    static async createReport(req, res) {
        try {
            ...
            const productoResult = await query('SELECT * FROM items WHERE id = $1', [item_id]);
            ...
            const reporteExistente = await query('SELECT * FROM reportes WHERE item_id = $1 AND usuario_reportador_id = $2', [item_id, usuario_reportador_id]);
            ...
            const result = await query(`INSERT INTO reportes 
        (item_id, usuario_reportador_id, tipo_reporte, descripcion, comentario_opcional, estado)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING *`, [item_id, usuario_reportador_id, tipo_reporte, motivo_reporte, informacion_adicional || null, 'pendiente']);
    ...
    static async getReportsByProduct(req, res) {
        ...
        WHERE r.item_id = $1
        ORDER BY r.fecha_reporte DESC`, [item_id]);
            res.json({
                success: true,
                data: result.rows,
    ...
    static async getPendingReports(req, res) {
        ...
        WHERE ${whereClause}
        ORDER BY r.fecha_reporte ASC`, queryParams);
            res.json({
                success: true,
                data: result.rows,
                count: result.rows.length
    ...
    static async resolveReport(req, res) {
        try {
            ...
            const reporteResult = await query('SELECT * FROM reportes WHERE id = $1', [report_id]);
            ...
            await query(`UPDATE reportes 
        SET estado = $1,
            decision_final = $2,
            moderador_resolutor_id = $3,
            fecha_revision = CURRENT_TIMESTAMP,
            fecha_resolucion = CURRENT_TIMESTAMP
        WHERE id = $4`, [nuevoEstadoReporte, decision_final, moderador_resolutor_id, report_id]);
        ...
        WHERE id = $5`, [nuevoEstadoProducto, esPeligroso, moderador_resolutor_id, accion !== 'aprobar' ? decision_final : null, reporte.item_id]);
            // Obtener información del producto para obtener el vendedor_id
            const productoResult = await query('SELECT vendedor_id FROM items WHERE id = $1', [reporte.item_id]);
            ...
            const reporteActualizado = await query(`SELECT 
          r.*,
          i.nombre as producto_nombre,
          i.estado as producto_estado,
          i.es_peligroso,
    ...
    static async getMyReports(req, res) {
        ...
        ORDER BY r.fecha_reporte DESC`, [usuario_id]);
            res.json({
                success: true,
                data: result.rows,
                count: result.rows.length
