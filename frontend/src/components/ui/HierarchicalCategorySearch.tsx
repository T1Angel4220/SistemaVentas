...
const HierarchicalCategorySearch: React.FC<HierarchicalCategorySearchProps> = ({
  ...
  React.useEffect(() => {
    if (categories && categories.length > 0) {
      ...
      console.log('⚠️ HierarchicalCategorySearch: No hay categorías disponibles');
    }
  }, [categories]);
  ...
  const organizeCategories = (categories: Category[]) => {
    if (!categories || categories.length === 0) {
      console.log('⚠️ No hay categorías para organizar');
      return [];
    }
    ...
    parentsByName.forEach((duplicates, name) => {
      if (duplicates.length > 1) {
        console.log(`⚠️ Categoría duplicada "${name}": ${duplicates.map(c => `ID ${c.id}`).join(', ')}`);
      }
    });
  ...
  const flatCategories = React.useMemo(() => {
    ...
    filteredCategories.forEach(parent => {
      ...
      if (shouldShowSubcategories && hasSubcategories) {
        ...
        console.log(`  ⏸️ ${parent.nombre} (ID: ${parent.id}) tiene ${parent.subcategorias.length} subcategorías pero no se están mostrando (expandida: ${isExpanded})`);
      } else {
  ...
  const toggleExpansion = (categoryId: number) => {
    ...
    if (!parentCategory) {
      ...
      console.log(`   ⚠️ No encontrada en filteredCategories, buscando en organizeCategories...`);
    }
    ...
    if (parentCategory) {
      ...
      console.log(`   ⚠️ Categoría NO encontrada en ninguna estructura`);
  ...
  useEffect(() => {
    ...
    if (isOpen && !searchTerm && filteredCategories.length > 0) {
      ...
      const parentIdsWithSubcategories = filteredCategories
        .filter(cat => {
          ...
          if (hasSubs) {
            ...
            console.log(`  ⚠️ ${cat.nombre} (ID: ${cat.id}) NO tiene subcategorías`);
          }
          return hasSubs;
        })
        .map(cat => cat.id);
      ...
      if (parentIdsWithSubcategories.length > 0) {
        ...
        console.log(`⚠️ No se encontraron categorías padre con subcategorías para expandir`);
      ...
      console.log(`⏸️ No se ejecuta auto-expansión: isOpen=${isOpen}, searchTerm="${searchTerm}", filteredCategories.length=${filteredCategories.length}`);
    }
